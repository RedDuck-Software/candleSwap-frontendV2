import { resolutionsForSocket } from './helpers.js'

const socket = new WebSocket('wss://candleswapwebsocket.azurewebsites.net/socket')
...
socket.onmessage = (data) => {
  ...
  if (
    data.data === 'Uniswap Heartbeat' ||
    data.data.includes('"event":"subscribe"')
  ) {
...
export function subscribeOnStream(
  ...
  const chanelMessage = JSON.stringify({
    event: 'subscribe',
    channel: 'candles',
    key: `trade:${resolutionsForSocket(resolution)}:${symbolInfo.token0Id}:${
      symbolInfo.token1Id
    }`,
  })
  const handler = {
